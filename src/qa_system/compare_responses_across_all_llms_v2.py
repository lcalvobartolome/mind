...
for llm_model in ["qwen:32b"]:
    ...
    for question_id, question_row in tqdm(df_info_topic.iterrows(), total=len(df_info_topic)):
        ...
        if question_id % 100 == 0:
            df_info_topic.to_excel(f"GENERATIONS/outs_good_model_tpc15_complete/questions_queries/questions_topic_{topic}_{llm_model}_{question_id}.xlsx")
            
    df_info_topic.to_excel(f"GENERATIONS/outs_good_model_tpc15_complete/questions_queries/questions_topic_{topic}_{llm_model}_all.xlsx")